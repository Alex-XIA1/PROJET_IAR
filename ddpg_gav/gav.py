...
def run_ddpg(cfg, logger, trial=None):
    ...
    while nb_steps < cfg.algorithm.n_steps:
        ...
        q_agent(transition_workspace, t=0, n_steps=1, detach_actions=True)
        ...
        with torch.no_grad():
            ...
            target_q_agent(transition_workspace, t=1, n_steps=1, detach_actions=True)
            # q_agent(rb_workspace, t=1, n_steps=1)
        # finally q_values contains the above collection at t=0 and t=1
        post_q_values = transition_workspace["target-critic/q_values"]

        critic_loss = compute_critic_loss(
                    cfg, reward, must_bootstrap, q_values[0],post_q_values[1]
                )
        
        # Log le cout du critic et on met a jour
        ...
        q_values = transition_workspace["critic/q_values"]
        actor_loss = compute_actor_loss(q_values)
        # print("ACTEUR ",actor_loss)
